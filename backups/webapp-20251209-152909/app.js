...
async function loadAvailablePonds() {
  try {
    ...
    if (data.ponds) {
      ...
      console.log(
        `✓ Loaded ${data.total_ponds} data ponds:`,
...
function setupCollapsibleHeaders() {
  ...
  console.log(`✓ Setting up ${headers.length} collapsible headers`);
...
function setupTooltipDebug() {
  ...
  if (tooltipElements.length === 0) {
    console.error("❌ NO tooltip elements found! Check HTML.");
    return;
  }
  ...
  tooltipElements.forEach((el, index) => {
    ...
    el.addEventListener("mouseenter", function () {
      console.log(`🖱️ Tooltip ${index + 1} HOVERED:`, tooltipText);
      console.log(`  Position:`, this.getBoundingClientRect());
    });

    el.addEventListener("mouseleave", function () {
      console.log(`🖱️ Tooltip ${index + 1} left`);
    });
  });

  // Force hide descriptions
  console.log("🔧 Force hiding pond descriptions...");
...
// =============================================================================
// METADATA FUNCTIONS - Added for real-time pond statistics
// =============================================================================

async function loadPondMetadata(pond = null) {
  try {
    console.log(`Fetching metadata${pond ? ' for ' + pond : ''}...`);
    const response = await fetch(CONFIG.API_BASE_URL + CONFIG.PLAINTEXT_ENDPOINT, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({action: 'get_metadata', pond: pond})
    });
    if (response.ok) {
...
async function displayPondMetadata(pondName) {
  const loadingId = showLoading();
  const metadata = await loadPondMetadata(pondName);
  removeLoading(loadingId);
  if (!metadata || !metadata.ponds[pondName]) {
    addMessage('bot', '<div>Could not fetch metadata for ' + pondName + '</div>');
    return;
  }
  const pond = metadata.ponds[pondName];
  const collectionTime = new Date(metadata.collection_timestamp).toLocaleString();
  let html = '<div class="markdown-content">';
  html += '<h2><i class="fas fa-database"></i> ' + pondName.toUpperCase() + ' Pond Metadata</h2>';
  html += '<p style="opacity: 0.7; font-size: 12px;">Collected: ' + collectionTime + '</p>';
  html += '<h3>Summary</h3>';
  html += '<div class="data-display" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">';
  html += '<div class="data-stat"><div class="data-stat-label">Files</div><div class="data-stat-value">' + pond.total_files.toLocaleString() + '</div></div>';
  html += '<div class="data-stat"><div class="data-stat-label">Size</div><div class="data-stat-value">' + pond.total_size_gb + ' GB</div></div>';
  html += '<div class="data-stat"><div class="data-stat-label">Freshness</div><div class="data-stat-value">' + (pond.freshness_minutes || 'N/A') + ' min</div></div>';
  html += '</div></div>';
  addMessage('bot', html);
}

async function displayAllPondsMetadata() {
  const loadingId = showLoading();
  const metadata = await loadPondMetadata();
  removeLoading(loadingId);
  if (!metadata) {
    addMessage('bot', '<div>Could not fetch metadata</div>');
    return;
  }
  const collectionTime = new Date(metadata.collection_timestamp).toLocaleString();
  let html = '<div class="markdown-content">';
  html += '<h2><i class="fas fa-database"></i> Data Lake Overview</h2>';
  html += '<p style="opacity: 0.7; font-size: 12px;">Collected: ' + collectionTime + '</p>';
  html += '<h3>Overall Statistics</h3>';
  html += '<div class="data-display" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;">';
  html += '<div class="data-stat"><div class="data-stat-label">Ponds</div><div class="data-stat-value">' + metadata.summary.total_ponds + '</div></div>';
  html += '<div class="data-stat"><div class="data-stat-label">Active</div><div class="data-stat-value">' + metadata.summary.active_ponds + '</div></div>';
  html += '<div class="data-stat"><div class="data-stat-label">Files</div><div class="data-stat-value">' + metadata.summary.total_files.toLocaleString() + '</div></div>';
  html += '<div class="data-stat"><div class="data-stat-label">Size</div><div class="data-stat-value">' + metadata.summary.total_size_gb + ' GB</div></div>';
  html += '</div><h3>Pond Details</h3>';
  Object.keys(metadata.ponds).forEach(function(pondName) {
    const pond = metadata.ponds[pondName];
    html += '<div style="background: rgba(0,0,0,0.2); padding: 10px; margin: 10px 0; border-radius: 6px;">';
    html += '<h4>' + pondName.toUpperCase() + '</h4>';
    html += '<p>Files: ' + pond.total_files.toLocaleString() + ' | Size: ' + pond.total_size_gb + ' GB | Updated: ' + (pond.freshness_minutes !== undefined ? pond.freshness_minutes + ' min ago' : 'N/A') + '</p>';
    html += '</div>';
  });
  html += '</div>';
  addMessage('bot', html);
