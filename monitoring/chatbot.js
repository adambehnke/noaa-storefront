...
function setupEventListeners() {
  ...
  // Pond selector
  elements.pondOptions.forEach((pondOption) => {
    pondOption.addEventListener("click", () => {
      const pond = pondOption.dataset.pond;
      selectPond(pond);
    });

    // Add double-click handler to show recent data examples
    pondOption.addEventListener("dblclick", () => {
      const pond = pondOption.dataset.pond;
      if (pond !== "all") {
        console.log(`Double-clicked pond: ${pond}`);
...
function populateEndpointsPanel(ponds) {
      ...
      html += `
        <div style="margin: 5px 0; padding: 8px 10px; background: rgba(0,0,0,0.2); border-radius: 3px; cursor: pointer;"
          ...
          <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="font-size: 9px; display: flex; gap: 8px;">
              <span style="opacity: 0.6; color: #0066cc;" class="endpoint-query-btn" data-pond="${pondName}" data-service="${endpoint.service}" data-endpoint="${endpoint.name}">
                <i class="fas fa-play-circle"></i> Query
              </span>
              <span style="opacity: 0.6; color: #28a745;" class="endpoint-recent-btn" data-pond="${pondName}" data-service="${endpoint.service}" data-endpoint="${endpoint.name}">
  ...
  // Add click handlers to endpoint query buttons
  document.querySelectorAll(".endpoint-query-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      console.log(
        `Endpoint query clicked: ${pond} - ${service} - ${endpointName}`,
      );
      queryEndpointDirectly(pond, service, endpointName);
    });
  });

  // Add click handlers to endpoint recent data buttons
  document.querySelectorAll(".endpoint-recent-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      e.stopPropagation();
      const pond = this.getAttribute("data-pond");
      const service = this.getAttribute("data-service");
      const endpointName = this.getAttribute("data-endpoint");
      console.log(
        `Recent data clicked: ${pond} - ${service} - ${endpointName}`,
      );
      fetchRecentEndpointData(pond, service, endpointName);
    });
  });

  // Add click handlers to endpoint items (for fallback)
  document.querySelectorAll(".endpoint-item").forEach((item) => {
    item.addEventListener("click", function (e) {
      // Only trigger if not clicking on a button
      if (
        !e.target.closest(".endpoint-query-btn") &&
        !e.target.closest(".endpoint-recent-btn")
      ) {
        const pond = this.getAttribute("data-pond");
        const service = this.getAttribute("data-service");
        const endpointName = this.getAttribute("data-endpoint");
...
// Fetch recent ingested data for a specific endpoint
async function fetchRecentEndpointData(pond, service, endpointName) {
  console.log(`Fetching recent data for: ${pond}/${service}/${endpointName}`);

  // Add user message
  addMessage("user", `Show recent data: ${endpointName} (${pond})`);
  ...
  try {
    // Query S3 to get the most recent file from bronze layer
    const response = await fetch(`${CONFIG.API_BASE_URL}/query`, {
      ...
      body: JSON.stringify({
        query: `Get the most recent example of data ingested for ${endpointName} in the ${pond} pond`,
        ponds: [pond],
      }),
    });
    ...
    // Build response message
    let messageHTML = "<div>";
    messageHTML += `<h4><i class="fas fa-clock"></i> Recent Ingested Data</h4>`;
    messageHTML += `<div style="margin: 10px 0; padding: 10px; background: rgba(40,167,69,0.1); border-radius: 6px;">`;
    messageHTML += `<strong>Endpoint:</strong> ${endpointName}<br>`;
    messageHTML += `<strong>Service:</strong> ${service}<br>`;
    messageHTML += `<strong>Pond:</strong> ${pond}<br>`;

    // Extract latest ingestion time from pond metadata
    if (state.availablePonds && state.availablePonds[pond]) {
      const pondData = state.availablePonds[pond];
      if (pondData.latest_ingestion) {
        const ingestTime = new Date(pondData.latest_ingestion);
        const now = new Date();
        const minutesAgo = Math.floor((now - ingestTime) / 60000);
        messageHTML += `<strong>Latest Ingestion:</strong> ${ingestTime.toLocaleString()} (${minutesAgo} minutes ago)<br>`;
      }
      if (pondData.total_files) {
        messageHTML += `<strong>Total Files:</strong> ${pondData.total_files.toLocaleString()}<br>`;
      }
    }
    ...
    if (data && data.answer) {
      messageHTML += `<div style="margin: 10px 0;">`;
      messageHTML += `<strong><i class="fas fa-database"></i> Recent Data Sample:</strong>`;
      messageHTML += `<div style="padding: 10px; background: rgba(0,0,0,0.2); border-radius: 4px; margin-top: 5px;">`;
      messageHTML += `<div class="markdown-content" style="white-space: pre-wrap; font-size: 11px;">${escapeHtml(data.answer)}</div>`;
      messageHTML += `</div></div>`;
    }

    // Show Bronze layer info
    messageHTML += `<div style="margin: 10px 0; padding: 10px; background: rgba(165,42,42,0.1); border-radius: 4px;">`;
    messageHTML += `<strong><i class="fas fa-layer-group"></i> Bronze Layer:</strong><br>`;
    messageHTML += `<div style="font-size: 11px; opacity: 0.9; margin-top: 5px;">`;
    messageHTML += `Raw ingested data is stored in: <code style="font-size: 10px;">s3://bucket/bronze/${pond}/</code><br>`;
    messageHTML += `Files are organized by data type and date for efficient retrieval.`;
    messageHTML += `</div></div>`;
    ...
    let errorHTML = "<div>";
    errorHTML += `<h4><i class="fas fa-exclamation-triangle"></i> Failed to Fetch Recent Data</h4>`;
    errorHTML += `<div style="color: #f44336; padding: 10px; background: rgba(244,67,54,0.1); border-radius: 6px;">`;
    errorHTML += `<strong>Error:</strong> ${error.message}<br>`;
    errorHTML += `<strong>Endpoint:</strong> ${endpointName}<br>`;
    errorHTML += `<strong>Service:</strong> ${service}`;
    errorHTML += `</div>`;
    errorHTML += `<div style="margin-top: 10px; opacity: 0.7; font-size: 11px;">`;
    errorHTML += `Unable to retrieve recent ingested data. This may be due to no recent ingestions or temporary access issues.`;
    errorHTML += `</div></div>`;

    addMessage("bot", errorHTML);

    console.error("Recent data fetch failed:", error);
  } finally {
    state.isProcessing = false;
  }
}

// Show comprehensive data examples for all endpoints in a pond
async function showPondDataExamples(pondName) {
  console.log(`Showing data examples for pond: ${pondName}`);

  // Add user message
  addMessage("user", `Show all recent data examples for ${pondName} pond`);
  ...
  try {
    // Get pond info from state
    const pondData = state.availablePonds
      ? state.availablePonds[pondName]
      : null;
    const pondInfo = FALLBACK_PONDS[pondName] || {};

    // Build response message
    let messageHTML = "<div>";
    messageHTML += `<h4><i class="fas fa-database"></i> ${pondInfo.name || pondName.charAt(0).toUpperCase() + pondName.slice(1)} Pond - Recent Data Examples</h4>`;

    if (pondData) {
      messageHTML += `<div style="margin: 10px 0; padding: 10px; background: rgba(0,102,204,0.1); border-radius: 6px; font-size: 12px;">`;
      messageHTML += `<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">`;

      messageHTML += `<div><strong>Total Files:</strong> ${pondData.total_files ? pondData.total_files.toLocaleString() : "N/A"}</div>`;
      messageHTML += `<div><strong>Total Size:</strong> ${pondData.total_size_gb ? pondData.total_size_gb.toFixed(2) + " GB" : "N/A"}</div>`;

      if (pondData.latest_ingestion) {
        const ingestTime = new Date(pondData.latest_ingestion);
        const now = new Date();
        const minutesAgo = Math.floor((now - ingestTime) / 60000);
        messageHTML += `<div><strong>Last Update:</strong> ${minutesAgo} min ago</div>`;
      } else {
        messageHTML += `<div><strong>Last Update:</strong> N/A</div>`;
      }

      messageHTML += `</div></div>`;
    }

    // List all endpoints with their recent data
    const endpoints = pondInfo.endpoints || [];

    if (endpoints.length > 0) {
      messageHTML += `<div style="margin: 15px 0;">`;
      messageHTML += `<strong><i class="fas fa-plug"></i> Active Endpoints (${endpoints.length}):</strong>`;
      messageHTML += `<div style="margin-top: 10px;">`;

      for (const endpoint of endpoints) {
        messageHTML += `<div style="margin: 10px 0; padding: 12px; background: rgba(0,0,0,0.2); border-left: 3px solid #0066cc; border-radius: 4px;">`;
        messageHTML += `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">`;
        messageHTML += `<strong><i class="fas fa-circle" style="font-size: 6px; margin-right: 8px; color: #28a745;"></i>${endpoint.name}</strong>`;
        messageHTML += `<div style="font-size: 10px; opacity: 0.7;">`;
        messageHTML += `<span style="background: rgba(0,102,204,0.3); padding: 2px 6px; border-radius: 3px;">${endpoint.service}</span>`;
        messageHTML += `</div></div>`;

        // Show endpoint details
        messageHTML += `<div style="font-size: 11px; opacity: 0.8; margin: 5px 0;">`;
        messageHTML += `<strong>API Path:</strong> <code style="font-size: 10px; background: rgba(0,0,0,0.3); padding: 2px 4px; border-radius: 2px;">${endpoint.url}</code>`;
        messageHTML += `</div>`;

        // Show data type info based on endpoint
        messageHTML += `<div style="font-size: 11px; opacity: 0.8; margin: 8px 0 0 0; padding: 8px; background: rgba(40,167,69,0.1); border-radius: 3px;">`;
        messageHTML += `<strong><i class="fas fa-info-circle"></i> Data Type:</strong> `;

        // Customize message based on endpoint type
        if (pondName === "atmospheric") {
          if (endpoint.name.includes("Alert")) {
            messageHTML += `Weather alerts, warnings, and advisories in GeoJSON format`;
          } else if (endpoint.name.includes("Forecast")) {
            messageHTML += `Weather forecasts with temperature, precipitation, and wind data`;
          } else if (endpoint.name.includes("Observation")) {
            messageHTML += `Current weather observations from NOAA weather stations`;
          } else {
            messageHTML += `Atmospheric and weather-related data from NWS`;
          }
        } else if (pondName === "oceanic") {
          if (endpoint.name.includes("Water Level")) {
            messageHTML += `Real-time water level measurements in meters`;
          } else if (endpoint.name.includes("Prediction")) {
            messageHTML += `Tide predictions for specified time periods`;
          } else if (endpoint.name.includes("Current")) {
            messageHTML += `Ocean current speed and direction data`;
          } else if (endpoint.name.includes("Temperature")) {
            messageHTML += `Water or air temperature readings in Celsius`;
          } else {
            messageHTML += `Coastal and oceanic measurement data from CO-OPS`;
          }
        } else if (pondName === "buoy") {
          messageHTML += `Marine buoy observations including wave height, wind, temperature, and pressure`;
        } else if (pondName === "climate") {
          messageHTML += `Historical climate data and statistics from weather stations`;
        } else if (pondName === "spatial") {
          messageHTML += `Radar and satellite imagery metadata and products`;
        } else if (pondName === "terrestrial") {
          messageHTML += `Stream flow, water level, and river gauge measurements`;
        } else {
          messageHTML += `NOAA data from ${endpoint.service} service`;
        }

        messageHTML += `</div>`;

        // Add action buttons
        messageHTML += `<div style="margin-top: 10px; display: flex; gap: 8px;">`;
        messageHTML += `<button class="action-btn" onclick="queryEndpointDirectly('${pondName}', '${endpoint.service}', '${endpoint.name}')" style="font-size: 10px; padding: 4px 8px;">`;
        messageHTML += `<i class="fas fa-play-circle"></i> Query Live`;
        messageHTML += `</button>`;
        messageHTML += `<button class="action-btn" onclick="fetchRecentEndpointData('${pondName}', '${endpoint.service}', '${endpoint.name}')" style="font-size: 10px; padding: 4px 8px;">`;
        messageHTML += `<i class="fas fa-clock"></i> View Recent`;
        messageHTML += `</button>`;
        messageHTML += `</div>`;
      ...
      messageHTML += `<div style="margin: 15px 0; padding: 10px; opacity: 0.7; font-size: 11px;">`;
      messageHTML += `No endpoints configured for this pond.`;
      messageHTML += `</div>`;
    }

    // Add layer information if available
    if (pondData && pondData.layers) {
      messageHTML += `<div style="margin: 15px 0; padding: 10px; background: rgba(255,193,7,0.1); border-radius: 6px;">`;
      messageHTML += `<strong><i class="fas fa-layer-group"></i> Data Lake Layers:</strong>`;
      messageHTML += `<div style="margin-top: 10px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; font-size: 11px;">`;

      ["bronze", "silver", "gold"].forEach((layer) => {
        if (pondData.layers[layer]) {
          const layerData = pondData.layers[layer];
          const layerColor =
            layer === "bronze"
              ? "#cd7f32"
              : layer === "silver"
                ? "#c0c0c0"
                : "#ffd700";
          messageHTML += `<div style="padding: 8px; background: rgba(0,0,0,0.2); border-left: 3px solid ${layerColor}; border-radius: 3px;">`;
          messageHTML += `<div style="font-weight: bold; text-transform: capitalize; margin-bottom: 5px;">${layer} Layer</div>`;
          messageHTML += `<div>Files: ${layerData.file_count ? layerData.file_count.toLocaleString() : "N/A"}</div>`;
          messageHTML += `<div>Size: ${layerData.size_mb ? (layerData.size_mb / 1024).toFixed(2) + " GB" : "N/A"}</div>`;
          if (layerData.latest_file) {
            const fileTime = new Date(layerData.latest_file);
            const minutesAgo = Math.floor((new Date() - fileTime) / 60000);
            messageHTML += `<div>Updated: ${minutesAgo}m ago</div>`;
          }
          messageHTML += `</div>`;
        }
      });
    ...
    // Add helpful tips
    messageHTML += `<div style="margin: 15px 0; padding: 10px; background: rgba(0,0,0,0.1); border-radius: 6px; font-size: 11px; opacity: 0.8;">`;
    messageHTML += `<strong><i class="fas fa-lightbulb"></i> Tips:</strong><br>`;
    messageHTML += `• Click "Query Live" to fetch fresh data directly from the NOAA API<br>`;
    messageHTML += `• Click "View Recent" to see the most recently ingested data from the Data Lake<br>`;
    messageHTML += `• Double-click any pond in the sidebar to view its data examples`;
    messageHTML += `</div>`;
    ...
    let errorHTML = "<div>";
    errorHTML += `<h4><i class="fas fa-exclamation-triangle"></i> Failed to Load Pond Examples</h4>`;
    errorHTML += `<div style="color: #f44336; padding: 10px; background: rgba(244,67,54,0.1); border-radius: 6px;">`;
    ...
    console.error("Pond examples failed:", error);
  } finally {
    state.isProcessing = false;
  }
}
...
window.fetchRecentEndpointData = fetchRecentEndpointData;
window.showPondDataExamples = showPondDataExamples;
window.queryEndpointDirectly = queryEndpointDirectly;
